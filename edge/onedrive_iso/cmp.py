...
class docdiff:
    ...
    def diff_zips(self, doc1, doc2):
        ...
        old_file_dir = self._tmproot + '/old/' + docname
        new_file_dir = self._tmproot + '/new/' + docname
        self.diff_dirs(old_file_dir, new_file_dir)
        patch_root = './patch_dir/' + docname
    ...
    def patch_zips(self, old_doc, patch_doc):
        ...
        print(time.time())
        print("time on uncompress is {0}".format(str(time.time()-t)))
        ...
        ziph.close()
        print(time.time())
        print("time on compress is {0}".format(str(time.time()-t)))
            ...
            for ifile in files:
                ...
                if os.path.exists(tmproot+"/"+ifile):
                    ...
                    if not diff:
                        ...
                        # print flush_content
                        flush_content = self.PatchtoText(flush_content)
                        # print flush_content
    ...
    def patch_dirs(self, patch_dir, odname):
        ...
        print("time on dir {0} is {1}".format(odname, str(time.time()-t)))
    ...
    def patch_files(self, ofname, patch):
        ...
        f.truncate()
        f.close()
        print("time on {0} is {1}".format(ofname, str(time.time()-t)))
...
if __name__ == "__main__":
    d =docdiff()
    d.diff_zips("../res/old/1.docx", "../res/new/1.docx")
